...
DEPENDS += "virtual/kernel openssl armv8-crypto"
...
"
SRCREV = "66be730886ce7a1e5bbd3b333d7178491aa9392c"
...
EXTRA_OEMAKE += 'ARCH="${ARCH}" CROSS="${TARGET_PREFIX}" \
    ...
    ARMV8_CRYPTO_LIB_PATH="${STAGING_DIR_HOST}" \
'
...
do_install() {
    ...
    oe_runmake EXTRA_CFLAGS+="-Wno-error=deprecated-declarations  -fPIC" EXTRA_LDFLAGS="-L${STAGING_LIBDIR} --hash-style=gnu"  WERROR_FLAGS="-w" V=1  T="${RTE_TARGET}" DESTDIR="${D}" install CONFIG_RTE_KNI_KMOD=n CONFIG_RTE_LIBRTE_PMD_OPENSSL=y

    # Build and install the DPDK examples
    for APP in examples/l3fwd examples/l2fwd-crypto examples/ipsec-secgw; do
        temp=`basename ${APP}` 
